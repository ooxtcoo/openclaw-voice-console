...
function makeFaceGeometry(){
  // keep it very low-poly so the wireframe reads as a face, not a mesh-soup
  const geo = new THREE.SphereGeometry(0.9, 12, 9);
...
function rebuildFace(){
  ...
  face.geometry = makeFaceGeometry();
}
...
function eyeCurve(sign){
  ...
  for (let i=0;i<=18;i++){
    ...
    const x = sign * 0.30 + Math.cos(t) * s;
    const y = 0.13 + Math.sin(t) * (s * 0.33);
    const z = 0.73;
    pts.push(new THREE.Vector3(x,y,z));
  }
  return new THREE.BufferGeometry().setFromPoints(pts);
...
// centers
pupilL.position.set(-0.30, 0.13, 0.76);
pupilR.position.set(+0.30, 0.13, 0.76);
...
function animate(){
  ...
  if (typeof eyesGroup !== 'undefined' && eyesGroup) {
    ...
    if (typeof pupilL !== 'undefined' && pupilL) {
      pupilL.position.x = -0.30 + px;
      pupilL.position.y = 0.13 + py;
    }
    if (typeof pupilR !== 'undefined' && pupilR) {
      pupilR.position.x = +0.30 + px;
      pupilR.position.y = 0.13 + py;
    }
  }
...
async function boot(){
  ...
  function bindSlider(id, key, fmt=(v)=>String(v)){
    ...
    el.addEventListener('input', () => {
      ...
      // live apply
      if (key === 'wireOpacity') faceMat.opacity = settings.wireOpacity;
      ...
      if (key === 'mouthWidth' || key === 'mouthSmile') {
        // rebuild mouth
  ...
  const doOneUtterance = async () => {
    ...
    try {
      ...
      const wav = await recordOnce({maxMs:20000, vad:true});
      mode = 'thinking';
